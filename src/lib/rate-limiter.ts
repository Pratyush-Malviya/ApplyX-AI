...
function makeLimiter(redis: Redis, count: number): Ratelimit {
...
function getLimiters() {
  ...
  if (!_tailorResume) _tailorResume = makeLimiter(redis, 10);
  if (!_coverLetter)  _coverLetter  = makeLimiter(redis, 5);
  if (!_analyzeJob)   _analyzeJob   = makeLimiter(redis, 20);
  if (!_general)      _general      = makeLimiter(redis, 30);

  return { tailorResume: _tailorResume, coverLetter: _coverLetter, analyzeJob: _analyzeJob, general: _general };
}

// ─── Public API ───────────────────────────────────────────────────────────────

export type RateLimitKey = "resume" | "cover-letter" | "analyze" | "general";
